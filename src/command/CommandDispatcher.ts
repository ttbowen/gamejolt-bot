...
export class CommandDispatcher<T extends Client> {
  ...
  private async handleMessage(message: Message): Promise<void> {
    ...
    const [commandCalled, command, prefix, name]: [
      boolean,
      Command<T>,
      string,
      string
    ] = await this.isCommandCalled(message);
    ...
    if (
      (await this._client.isQuiet(message.roomId)) &&
    ...
    for (let m of middleware) {
      try {
        let result: Promise<[Message, any[]]> | [Message, any[]] = m.call(command, message, args);
    ...
    if (middlewarePassed) await this.dispatch(command, message, args).catch(console.error);
  ...
  private async isCommandCalled(message: Message): Promise<[boolean, Command<T>, string, string]> {
    ...
    if (typeof prefix === 'undefined' && message.isMentioned)
      prefix = message.contentRaw.split(' ')[0];
    if (pm && typeof prefix === 'undefined') prefix = '';
    if (typeof prefix === 'undefined' && !pm) return [false, null, null, null];

    const commandName: string = message.contentRaw
      .trim()
      .slice(prefix.length)
      .trim()
      .split(' ')[0];

    const command: Command<T> = this._client.commands.find(
      c =>
  ...
  private _checkRateLimits(message: Message, command?: Command<T>): boolean {
    ...
    if (passedRateLimiters) {
      if (
        !(command && command.rateLimiter && !command.rateLimiter.get(message).call()) &&
        this._client.rateLimiter
      )
  ...
  private _checkRateLimiter(message: Message, command?: Command<T>): boolean {
    const rateLimiter: CommandRateLimiter = command
      ? command.rateLimiter
      : this._client.rateLimiter;
    ...
    if (!rateLimit.wasNotified) {
      ...
      if (!command)
        message.reply(
          `Global cooldown. Try again in ${TimeUtil.difference(
            rateLimit.expires,
            Date.now()
          ).toString()}`
        );
        ...
        message.reply(
          `Command cooldown. Try again in ${TimeUtil.difference(
            rateLimit.expires,
            Date.now()
          ).toString()}`
        );
  ...
  private checkCallerPermissions(command: Command<T>, message: Message): Permissions[] {
    ...
    if (message.user.permissionLevel > 1) permissions.push(Permissions.SITE_MODERATOR);
