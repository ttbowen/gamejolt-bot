...
export default class extends Command {
  ...
  public async invoke(message: Message, [mode]: [Modes]): Promise<void> {
    if (!mode) {
      const currentMode: string = await this.client.getCurrentMode(message.roomId);
      return message.reply(`I am currently in \`${currentMode}\` mode`);
    }

    if (!mode.match(/\b(serious|fun|quiet|chatty)\b/)) {
      return message.reply(`Please pass a valid mode`);
    }
