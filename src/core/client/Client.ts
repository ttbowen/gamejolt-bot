...
export class Client extends GameJolt.Client {
    ...
    public constructor(botOptions: BotOptions, clientOptions?: ClientOptions) {
        ...
        this.owners = botOptions.ownerIds instanceof Array ?
            botOptions.ownerIds : typeof botOptions.ownerIds !== 'undefined' ? [botOptions.ownerIds] : [];
        ...
        if (typeof botOptions.logLevel !== 'undefined')
            this._logger.setLogLevel(botOptions.logLevel);
    ...
    public start(): this {
        ...
        if (!config.username || !config.password || !config) throw new Error('No username or password has been provided.');
    ...
    private async _enterDefaultRooms(): Promise<void> {
        for (let room of this._defaultRooms) {
            if (room)
                await this.chat.enterRoom(room);
        }
    }
    ...
    public on(event: 'command', listener: (name: string, args: any[], message: Message) => void): this;
